...
#define LCD_SCLopp  GPIO_QuickInit(HW_GPIOA, 26, kGPIO_Mode_OPP)  
#define LCD_SDAopp  GPIO_QuickInit(HW_GPIOA, 27, kGPIO_Mode_OPP)
#define LCD_RSTopp  GPIO_QuickInit(HW_GPIOA, 28, kGPIO_Mode_OPP)
#define LCD_DCopp   GPIO_QuickInit(HW_GPIOA, 29, kGPIO_Mode_OPP)

#define LCD_SCL  PAout(26)
#define LCD_SDA  PAout(27)
#define LCD_RST  PAout(28)
#define LCD_DC   PAout(29)
...
void DelayNs(uint32_t ns)
{
    ...
    startts = DWT->CYCCNT;
    ts =  ns * (SystemCoreClock /(1000*10000) ); 
    endts = startts + ts;      
    if(endts > startts)  
    {
...
//==============================================================
//函数名：fsLCD_P6x8Str(byte x,byte y,byte *p)
//功能描述：写入一组标准ASCII字符串
//参数：显示的位置（x,y），y为页范围0～7，要显示的字符串
//返回：无
//==============================================================  
...
//==============================================================
//函数名：LCD_P6x8Char(byte x,byte y,byte p)
//功能描述：写入一个标准ASCII字符串
...
//==============================================================
//函数名：  void LCD_PrintValueI(byte x, byte y, int data);
...
//==============================================================
//函数名：   void LCD_PrintValueF(byte x, byte y, float data, byte num);
...
//==============================================================
 ...
 {
 	byte l,m,i,j,k;  //万千百十个
 	byte databiti = 6; //整数位数
 	dword tempdataui = 0;
  	long tempdataii = (long)data; //整数部分
 	long int tempdatalp = (long int)((data - (long)data)*10000); //取小数位后4位
	...
	{
		case 0: break;
		case 1:  LCD_PrintValueFP(x + databiti * 6, y, (dword)(tempdatalp / 1000),num);break;
		case 2:  LCD_PrintValueFP(x + databiti * 6, y, (dword)(tempdatalp / 100),num);break;
		case 3:  LCD_PrintValueFP(x + databiti * 6, y, (dword)(tempdatalp / 10),num);break;
		case 4:  LCD_PrintValueFP(x + databiti * 6, y, (dword)(tempdatalp),num);break;					
	}
        
 }

/*函数名称:void OLed_DisplayF(void);
 *函数说明:显示所需参数,6列*8行为一个字符，y=0~7,x为前面字符*6
 *参数: void
 *返回值: void
 */ 
...
{
    char disbuff[20];
    sprintf(disbuff,"%0.2f",num);
    LCD_P6x8Str(x,y,(byte *)disbuff);   
} */
/*函数名称:void OLed_DisplayI(byte x,byte y,signed long num);
 *函数说明:显示所需参数,6列*8行为一个字符，y=0~7,x为前面字符*6
 *参数: void
 *返回值: void
 */ 
